...
bot.on("message", async (msg) => {
  ...
  if (message === "/start") {
    await bot.sendMessage(
      chatId,
      `*Привет, это бот который поможет сдать топку, просто спроси у меня, что тебя интересует. Вопросы пиши как можно подробнее, если ответ не устраивает переформулируй вопрос или уточни что именно нужно узнать. Удачи! \n поддержка битком - bc1qsjk5hjwggjmpt8klnexsug4getgktd7tmfga4j*`,
      { parse_mode: "Markdown" }
    );
    return;
  }
